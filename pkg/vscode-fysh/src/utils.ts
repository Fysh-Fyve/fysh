...
export class FyshUtils {
  ...
  constructor(context: ExtensionContext) {
    ...
    this.#binPath = join(context.globalStorageUri.fsPath, "bin");
  }

  async initialize() {
    this.#client = await createLSPClient();
  ...
  #installFysh() {
    const commandHandler = async () => {
      if (await findExecutable(exe, this.#binPath)) {
        window.showInformationMessage(`${exe} is installed!`);
        return;
      }
      await mkdir(this.#binPath, { recursive: true });
      ...
      await window.withProgress(
        ...
        (progress) => installWithProgress(progress, this.#binPath)
      );
  ...
  #execInterminal() {
    const commandHandler = async (file: Uri) => {
      console.log(file);
      const executable = await findExecutable(exe, this.#binPath);
      if (!executable) {
        if (this.#context.globalState.get("promptInstall") === false) {
      ...
      if (!this.#terminal || this.#terminal?.exitStatus) {
        ...
        this.#terminal = window.createTerminal({ name: "Fysh" });
      }
      this.#terminal.show();
      ...
      if (file?.fsPath) {
        this.#terminal.sendText(`${quote(executable)} ${quote(file.fsPath)}`);
      } else {
        await window.showWarningMessage(`File not found!`);
      }
    };

    return commands.registerCommand(
      FyshCommands.ExecInTerminal,
      commandHandler
    );
  }
  ...
  async #promptInstall() {
    ...
    if (res === "Do not show again") {
      ...
      return this.#context.globalState.update("promptInstall", false);
    } else if (res === "Install") {
      return commands.executeCommand(FyshCommands.InstallFysh, this);
    }
  }
      ...
      () => {
        ...
        if (isNaN(decimalNumber)) {
          window.showErrorMessage(
            "Selected text is not a valid decimal number."
          );
          return;
        }
