...
async function promptInstall(context: ExtensionContext, state: ExtensionState) {
  ...
  if (res === "Do not show again") {
    window.showInformationMessage(`If you change your mind, ${exe} can be installed from the command prompt.`);
...
function findExecutableOSArch(): string {
  ...
  if (!(platform in valid)) {
    throw new Error(`Unsupported platform '${platform}'. Must compile interpreter with Go.`);
  }
...
async function installWithProgress(progress: Progress, binPath: string) {
  ...
  const assets = await fetch("https://api.github.com/repos/Fysh-Fyve/fysh/releases/latest")
    .then((res) => res.json())
    .then((res) => {
      if (typeof res === "object" && "assets" in res && Array.isArray(res.assets)) {
        return res.assets;
      } else {
  ...
  const md5Hash = await fetch(assets.filter((a) => a.name.includes("md5"))[0].browser_download_url)
    .then((res) => res.text())
    .then((hash) => hash.trim());

  const archive: { name: string; browser_download_url: string } = assets.filter((a) => !a.name.includes("md5"))[0];
  ...
  const fileHash = await writeFile(archivePath, buffer).then(() => calculateMD5Hash(archivePath));
  ...
  if (extname(archivePath) === "zip") {
    ...
    return new Promise<void>((resolve, reject) => {
      const process = spawn("tar", ["-C", binPath, "-x", "-z", "-f", archivePath]);
...
async function createLSPClient() {
  ...
  const client = new LanguageClient("fyshls", "Fysh Language Server", serverOptions, clientOptions);
...
function addPynkHeart() {
  ...
  const tokenRules: { textMateRules?: { scope?: string }[] } = config.get("editor.tokenColorCustomizations");
  ...
  const updatedTokenSettings: { textMateRules?: object } = {
    ...tokenRules,
    textMateRules: (tokenRules.textMateRules || []).filter((c) => !scopesToReplace.has(c?.scope)).concat(newRules),
  };

  config.update("editor.tokenColorCustomizations", updatedTokenSettings, ConfigurationTarget.Global);

  const semanticRules: { rules?: object } = config.get("editor.semanticTokenColorCustomizations");
  ...
  config.update("editor.semanticTokenColorCustomizations", updatedSettings, ConfigurationTarget.Global);
}
 ...
 * */
async function findExecutable(exe: PathLike, ...paths: string[]): Promise<string | null> {
  const envPath = process.env.PATH || "";
  const envExt = process.env.PATHEXT || "";
  const pathDirs = envPath.replace(/["]+/g, "").split(delimiter).filter(Boolean).concat(paths);
...
export async function activate(context: ExtensionContext) {
  ...
  state.client?.start();
  context.subscriptions.push(execInterminal(context, state), installFysh(state));
  addPynkHeart();

  const disposable = commands.registerCommand("extension.convertDecimalToBinary", () => {
  ...
  context.subscriptions.push(
    commands.registerTextEditorCommand("extension.convertLineToBinary", (editor) => {
      const document = editor.document;
      const position = editor.selection.active;
  ...
  context.subscriptions.push(
    commands.registerCommand("type", async ({ text }) => {
      ...
      if (editor) {
        ...
        if ((text === " " || text === "\t") && isCursorAtEndOfDecimalNumber(editor.document, line, position)) {
...
function isCursorAtEndOfDecimalNumber(document: TextDocument, line: TextLine, position: Position): boolean {
...
function convertDecimalToBinaryString(decimalNumber: number): string {
  ...
  if (decimalNumber < 0) {
    binaryNumber = "<°" + binaryNumber.replace(/1/g, "}").replace(/0/g, ")") + "><";
  } else {
    binaryNumber = "><" + binaryNumber.replace(/1/g, "{").replace(/0/g, "(") + "°>";
  }
  return binaryNumber;
